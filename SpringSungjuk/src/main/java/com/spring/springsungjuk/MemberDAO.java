...
public class  MemberDAO {
	...
	public int insertMember(SungjukVO memberVO)
	{
	int result=0;
	...
	//수정 
	public int modifyMember(SungjukVO memberVO)//modifyMember
	{
	int result=0;
	...
	try 
	{
		...
		pstmt.setString(6, memberVO.getHakbun());
...
//수정완료
	public int userCheck(SungjukVO memberVO)
	{
		String dbpasswd="";
		int x = -1;
		try
		...
		pstmt=conn.prepareStatement("select*from mvc_member where hakbun=?");
		pstmt.setString(1, memberVO.getHakbun());
	...
	//조회
	public ArrayList<SungjukVO> getMemberlist()
	{
		SungjukVO vo=null;
		ArrayList<SungjukVO> member_list=null;
		
		try {
			conn= JDBCUtil.getConnection();
			
			pstmt= conn.prepareStatement("select * from mvc_member order by id");
			rs=pstmt.executeQuery();
			
			if (rs.next())
			{
				member_list=new ArrayList<SungjukVO>();
				do
				{
					vo=new SungjukVO();
	...
	//아이디 검색
	public SungjukVO selectMember(SungjukVO memberVO)
	{
		SungjukVO vo=null;
